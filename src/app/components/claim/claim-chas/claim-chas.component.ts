...
export class ClaimChasComponent implements OnInit {
  ...
  onSearchClaimClicked() {
    ...
    let clinicId = this.store.getClinic().id;
  ...
  mapPayloadToRows(payload: any[]) {
    ...
    const arr = payload.map(res => {
      ...
      return {
        ...
        plan: (res.medicalCoverageName ? res.medicalCoverageName : '') + ' - ' + (res.planName ? res.planName : '')
      };
    });
    return arr;
  }
  ...
  getRemark(claim) {
    ...
    if (submissionResult && submissionResult.statusCode && claimResult && claimResult.remark) {
      // show claim result
      if (submissionResult.statusCode === 'OK') {
