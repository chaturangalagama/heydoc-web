...
export class CaseManagerListComponent implements OnInit {
  ...
  setPage(pageInfo) {
    ...
    this.apiCaseManagerService.getCaseList(this.store.clinicId, this.searchBody, this.page).subscribe(pagedData => {
  ...
  populateData(data) {
    this.caseResponse = data.map((payload, index) => {
      const tempCase = {
        date:moment(moment(payload.createdDate, 'DD-MM-YYYYT00:00:00')).format('MM-DD-YYYY'),
